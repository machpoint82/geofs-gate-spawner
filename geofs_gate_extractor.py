...
def find_apt_dat_files(root_dir):
    """Recursively find every file literally named apt.dat (case-insensitive) under root_dir."""
    root = Path(root_dir)
    return sorted(p for p in root.rglob("*") if p.is_file() and p.name.lower() == "apt.dat")


def main():
    ap = argparse.ArgumentParser(description="Extract GeoFS gate positions from X-Plane apt.dat file(s)")
    ap.add_argument("--input", help="Path to a single apt.dat file (use with --icao)")
    ap.add_argument("--icao", help="Comma-separated ICAO codes to extract from --input, e.g. EGLL,KJFK,LFPG")
    ap.add_argument("--batch-dir", help=(
        "Instead of --input/--icao: point this at a folder containing several unzipped "
        "airport downloads (any nesting depth). Every apt.dat found inside will be "
        "processed automatically, with its airport(s) auto-detected from the file's own "
        "header row — no need to type each ICAO code by hand."
    ))
    ...
    if not args.batch_dir and not (args.input and args.icao):
        ap.error("either --batch-dir, or both --input and --icao, are required")

    data = {}
    reported_icaos = []

    if args.batch_dir:
        apt_files = find_apt_dat_files(args.batch_dir)
        if not apt_files:
            print(f"No apt.dat files found anywhere under {args.batch_dir}")
            return
        print(f"Found {len(apt_files)} apt.dat file(s) under {args.batch_dir}\n")
        for apt_file in apt_files:
            # No ICAO filter: pick up whatever airport(s) this file actually contains.
            file_data = parse_apt_dat(apt_file, wanted_icaos=None)
            for icao, gates in file_data.items():
                data[icao] = gates  # last one wins if the same ICAO appears twice
                reported_icaos.append(icao)
            names = ", ".join(file_data.keys()) or "(no airport header found)"
            print(f"{apt_file}: {names}")
    else:
        icaos = [x.strip() for x in args.icao.split(",") if x.strip()]
        data = parse_apt_dat(args.input, icaos)
        reported_icaos = icaos
    ...
    print()
    for icao in reported_icaos:
        count = len(data.get(icao, []))
        print(f"{icao}: {count} parking position(s) found")
        if count == 0:
            print(f"  -> not found. Check the ICAO code exists in this apt.dat, "
